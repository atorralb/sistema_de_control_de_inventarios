...
mysql_query("UPDATE entradas_y_salidas set movimiento='$movimiento2',
almacen1 ='$almacen1',
almacen2 ='$almacen2',
fecha ='$fecha2',
factura='$factura',
...
elseif(urlencode(@$_REQUEST['accion'])=="buscar"){
    $folio=mysql_real_escape_string($_REQUEST['folio']);
    $header = mysql_query("select f, fecha, movimiento, almacen1, almacen2, factura, concepto, ne, nr, na from entradas_y_salidas where f = '$folio' limit 1");  
    while($s = mysql_fetch_array($header))        {
...
$total_results = mysql_result(mysql_query("select COUNT(*) from entradas_y_salidas where f = '$folio'"),0); 
