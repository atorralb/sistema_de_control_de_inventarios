...
if(urlencode(@$_REQUEST['accion'])=="buscar"){
    ...
    $ti = mysql_query("create temporary table inventario select 
...
$inventario = mysql_query("select * from inventario inner join productos productos on inventario.cprov = productos.cprov and 
inventario.cprod = productos.cprod;");
while ($seleccion = mysql_fetch_array($inventario))
{
extract($seleccion);
ECHO "<tr><td>".$cprod."</td><td>".$descripcion."</td><td>".$inventarioinicial."</td><td>".$entradas."</td><td>".$salidas."</td><td>".$inventariofinal."</td></tr>";
}

$result2 = mysql_query($inventario) or die($inventario."<br/><br/>".mysql_error());
}
?>
</table>
