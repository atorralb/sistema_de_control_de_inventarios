...
class Login {
    ...
    private function loginWithPostData() {
            ...
            $this->user_name = $this->connection->real_escape_string($_POST['user_email']);            
            $checklogin = $this->connection->query("SELECT user_name, user_email, user_password_hash FROM users WHERE user_email = '".$this->user_name."';");
            ...
            if($checklogin->num_rows == 1) {
               ...
               if ($_POST['user_password'] == $result_row->user_password_hash) {
                     ...
                     */
                    $log = ORM::for_table('user_records')->create();
                    $log->action = 'ENTRADA';
                    $log->who = $result_row->user_name;
                    $log->save();
